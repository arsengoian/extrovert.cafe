...
//
// Без «Привіт! Це підтримка extrovert.cafe» на початку (23.09.2026): рівно
// над цим повідомленням Telegram показує опис бота, який каже те саме, і
// два вітання поспіль лише ховали головне — чи бачимо ми акаунт.
const GREETING_LINKED = `Бачимо, з якого ви акаунта — розповідати про себе не треба.

Опишіть, що сталось: що замовляли, приблизно коли і що пішло не так. Є фото — прикріпіть, часто це найшвидший спосіб усе пояснити.
...
Якщо про автомат чи точку — пишіть тут. Підкажіть, де стоїть автомат і приблизно коли це було.`;

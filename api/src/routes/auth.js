...
// Нікнейм автогенерується при реєстрації (db-schema §1). Беремо теплі
// кавові слова, щоб екран "Твій нікнейм" одразу мав що показати.
const WORDS = ["арабіка", "крема", "зерно", "пінка", "бариста", "ристрето", "мокко", "лате", "фільтр", "темпер"];
...
export default async function routes(app) {
  ...
  app.post("/auth/dev", async (req, reply) => {
    ...
    const nickname = (req.body?.nickname || "").trim() || randomNickname();

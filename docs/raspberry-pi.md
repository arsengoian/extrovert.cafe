...
> ⚠️ **«Квадрат усе одно видно» (власник, 24.09.2026).** Райдужних квадратів
> два, і лікуються вони по-різному:
>
> - **на весь екран, одразу після подачі живлення, на пів секунди** — заставка
>   прошивки; її і гасить `disable_splash=1`. Якщо вона лишилась, `config.txt`
>   не той: картка розмічена NOOBS, і `/boot` тут — **p6**; рядок мусить бути
>   саме у файлі, який читає прошивка (`mount | grep boot`, далі `grep
>   disable_splash /boot/config.txt`).
> - **маленький, у правому верхньому кутку, зʼявляється під час роботи** —
>   це не заставка, а **попередження про просідання живлення**. Ховати його
>   (`avoid_warnings=1`) не можна: саме воно попереджає, що блок живлення чи
>   кабель не тягнуть, а запис на картку при просіданні — найчастіша причина
>   її смерті (§6-тер). Тепер це видно й у телеметрії: `throttled` і алерт
>   «⚡ просідає живлення».
>
> Щоб не гадати: `vcgencmd get_throttled`. `0x0` — живлення в нормі, і тоді
> квадрат таки заставка; будь-що інше — блок живлення.
...
Телеметрія йде **раз на хвилину**, overseer обходить перевірки теж раз на
хвилину, мовчання довше за **3 хвилини** вважається падінням. Тобто типова
затримка від поломки до повідомлення в Telegram — **1–4 хвилини**.
...
| Флешка під відео відвалилась | вийняти USB | точка монтування зникла або не пишеться | у телеметрії `usb_ok=false` |
...
**Чого ми не бачимо — і чому.** Монітор у standby по HDMI не відрізнити від
увімкненого: і hotplug, і EDID лишаються на місці (а в нас ще й
`hdmi_force_hotplug=1`, тобто малина видає сигнал навіть у порожнечу).
Єдиний спосіб спитати сам монітор — **CEC**, і телеметрія його пробує
(`cec-client -s`, пакет `cec-utils`). Якщо монітор CEC не вміє — у метриці
чесно стоїть `null`, і алерту немає: «не знаю» краще за вигадане
«увімкнено». Це й пояснює 24.09.2026: власник вимкнув монітор кнопкою, а
дашборд писав «працює», бо дивився на `tvservice -s`, тобто на власний
вихід малини.

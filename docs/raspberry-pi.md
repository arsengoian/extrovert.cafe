...
**Точку не можна штовхнути ззовні — вона ходить сама** (25.09.2026). CI не
має як сказати малині «є новий реліз»: вона стоїть за роутером у локальній
мережі, і маршруту до неї з GitHub Actions немає. Єдине, що визначає
затримку «задеплоїли → приїхало на точку», — це період опитування
маніфесту, а коштує воно нічого: `updater.sh` питає з `If-None-Match`, і
без змін бакет віддає 304 на пару сотень байтів. Тому період — **2 хвилини**
(`UPDATE_PERIOD_S`), а не пʼятнадцять, як було: чверть години очікування
була платою ні за що.

Якщо колись знадобиться реакція за секунди, канал уже є — кіоск тримає ws
до прода. Але тоді логіка оновлення переїде в кіоск (C, свій реліз на
кожну правку), а зараз нею завідує апдейтер, і це правильний власник.

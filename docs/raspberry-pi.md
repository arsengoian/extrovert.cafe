...
| | |
|---|---|
| Модель | Toshiba `SA16G`, 16 ГБ (14,5 ГіБ) |
| Дата випуску | **12/2017** — тобто їй девʼятий рік |
| Блок стирання | 4 МБ (`preferred_erase_size`) |
| Розмітка | NOOBS: p1 RECOVERY 1,8 ГБ · p5 32 МБ · p6 `/boot` 69 МБ · p7 `/` 12,7 ГБ |
| Вільно на корені | 8,2 ГБ із 12,7 |
...
2. Swap увімкнений (99 МБ, `dphys-swapfile`), але за весь аптайм **не
   свопнуло жодної сторінки** (`vmstat -s`). Тобто зараз він картку не
   зношує; вимикати не поспішаємо, але при переході на overlayfs його там
   бути не повинно.

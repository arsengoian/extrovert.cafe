...
розбудив панель за секунди — перевірено там же. Телеметрія робить це сама,
коли бачить standby, не частіше ніж раз на 10 хвилин
(`MONITOR_AUTOWAKE=0` у `config/env` вимикає), і позначає в пробі
`monitor_woke`. Тобто «вимкнули екран» тепер не лише видно, а й лікується
без виїзду.

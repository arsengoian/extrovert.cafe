...
- ~~**Кадр стоїть, поки рендериться меню**~~ — зроблено 24.09.2026:
  рендер переїхав у фоновий потік, головний лише заливає готову поверхню
  в текстуру. Але сам рендер на точці виявився **32,8 с**, а не ~8 с
  (заміряно 25.09.2026 у лозі кіоска). Кадр при цьому не стоїть, тож
  клієнт нічого не бачить, — але тридцять секунд одного ядра Pi 1 на
  перемальовування тринадцяти карток означає, що там є що чистити:
  найімовірніше rsvg і те, що перемальовуються ВСІ картки, а не змінені.

...
```
CI кладе маніфест у бакет
   └─ scheduler (jobs/release.js, раз на 30 с, If-None-Match)
        бачить, що release змінився
        └─ outbox → point:<id> → ws
             └─ кіоск (ws.c) торкається state/check-now
                  └─ updater.sh виходить з очікування за ≤5 с
```

Разом — під хвилину від публікації до початку оновлення. Кіоск тут лише
листоноша: оновленнями завідує апдейтер, просто ws тримає саме кіоск.

Сторожем працює планувальник, а не крок у CI, — свідомо: ніяких секретів і
ніякого нового ендпоїнта, і працює це для БУДЬ-ЯКОГО способу публікації,
навіть якщо маніфест колись покладуть повз GitHub Actions. Перший запуск із
порожнім Redis нічого не розсилає, інакше кожен деплой самого планувальника
гнав би точки перевіряти те, що в них уже стоїть.

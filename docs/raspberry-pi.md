...
корені: після кожного завантаження годинник стоїть у минулому, поки NTP не
підхопить (секунди при живому інтернеті). Небезпечно це лише в парі «немає
живлення + немає інтернету»: проби, що лягають у чергу до синхронізації,
отримають брехливий `measured_at` — рівно та хвороба, від якої лікували
графіки 25.09.2026. Лікується не карткою, а телеметрією: не довіряти
годиннику, поки він не пізніший за останню відому мітку на `data`.

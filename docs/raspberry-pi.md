...
graph TB
  ...
  subgraph dop["DigitalOcean · дроплет public"]
    api["api<br/>JWT пристрою, телеметрія,<br/>тимчасові ключі R2"]
  ...
  kiosk -- "меню точки після menu.deployed" --> worker
  worker --> r2
...
| `config/device.key` | адмінка → малина (ротація через обмін токена, перший ключ — по SSH) → кіоск, recorder | ключ можна відкликати й замінити без поїздки; у релізі й `config/env` його немає, бо релізи публічні (`services.md` §3) |
...
в коридорі немає і не має бути ключів до сховища релізів (recorder отримує
лише тимчасовий ключ на префікс відео своєї точки, `services.md` §3). Точка **сама забирає**
...
5. Перевірити кадри: `nc -U /tmp/pos-native-0.sock`.
   Коли зʼявиться `api` з пристроями: ключ пристрою з адмінки покласти по
   SSH у `/home/pi/extrovert/config/device.key` з правами `0600` — це
   єдиний раз, коли ключ їде не через ротацію.

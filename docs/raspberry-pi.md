...
| `updater.sh` | раз на 2 хв питає маніфест, привозить, перевіряє, підміняє, відкочує | **на пристрої**, але маніфест у бакеті — 404 (домен не привʼязаний), тож апдейтер тихо чекає |
...
| `state/check-now` | людина → апдейтер | перевірити оновлення зараз, а не чекати кола |
...
- `/etc/fstab` — рядки флешки `/mnt/buf` і розділу `data` (§5);
- `/sbin/overlayRoot.sh` — read-only корінь; запускається з `cmdline.txt` (§5);
...
sequenceDiagram
  ...
  loop раз на 2 хв або state/check-now
...
**4. Дочекатись або підштовхнути.** Точка сама перевірить протягом 2 хв
...
Розмітити картку на живій точці не можна: корінь змонтований і займає весь
залишок. Тому все робилося на ПК через кардрідер і на **новій картці**, а
стара лишилась цілою — вона ж і є головний аварійний вихід, якщо overlay
колись не підніметься.

Розкладка картки kyiv-01 (номери «дірчасті», бо NOOBS):

| Розділ | Що | Розмір |
|---|---|---|
| p1 | RECOVERY — звідси вантажиться прошивка (§1) | 1,7 ГБ |
| p5 | NOOBS SETTINGS | 32 МБ |
| p6 | `/boot` — **пишеться**, звідси керуємо overlay | 69 МБ |
| p7 | `/` — **read-only**, нижній шар overlay | 6 ГіБ |
| p8 | `data` → `/home/pi/extrovert` — єдине місце запису | 6,6 ГБ |

Корінь ужато з 12,7 ГБ до 6 ГіБ (зайнято 3,3), решта віддана під `data`:
зношується рівно та частина картки, у яку пишуть, тож що більший розділ, то
довше він живе (§6-тер). У fstab він записаний як `LABEL=data`, а не
`/dev/mmcblk0p8`: номер розділу залежить від розмітки, мітка — ні.

Механізм — **не** `raspi-config` (у цій збірці пункту overlayfs немає) і не
initramfs, а `init=/sbin/overlayRoot.sh` у `cmdline.txt`
(`../raspberry/pi/overlayRoot.sh`). Ядро запускає цей скрипт замість systemd:
він підкладає під корінь tmpfs через overlayfs, робить `pivot_root` і лише
тоді віддає керування справжньому init.

**Як вимкнути overlay, не розбираючи точку.** `/boot` лишився записуваним,
тож увесь цикл робиться по ssh:

```bash
sudo sed -i 's| init=/sbin/overlayRoot.sh||' /boot/cmdline.txt && sudo reboot
```

Точка підніметься зі звичайним записуваним коренем: правиш що треба,
повертаєш рядок, ребут. Те саме дає слово `nooverlay`, дописане в той самий
рядок, — скрипт бачить його й одразу віддає керування системі. Третій шлях,
коли ssh недоступний, — вставити запасну картку.

Правити систему «крізь» працюючий overlay (перемонтувати `/mnt/lower` у rw)
**не треба**: overlayfs не підтримує зміну нижнього шару під змонтованим
overlay, і наслідки там не визначені.

Релізів усе це не стосується — вони їдуть на `data` і ставляться, поки
...
копії — §2): що рідше туди лізти, то дешевше overlay.

### Що перевірено наживо

Після першого завантаження нової картки й після ребуту:

- `mount` показує `overlay on / type overlay`, `/mnt/lower` — `ro`;
- `sudo touch /mnt/lower/…` → `Read-only file system`: картка захищена
  навіть від root;
- файл, створений у `/etc`, після ребуту **зник**; файл на `data` **лишився**;
- стек піднявся сам (kiosk, supervisor, updater, telemetry), телеметрія
  шле щохвилини, апдейтер опитує маніфест, ws підʼєднаний.

### Три помилки, знайдені до першого запуску

Усі три — в аварійній гілці скрипта, і кожна сама по собі означала б точку
без системи. Записані тут, бо клас помилки повторюваний.

1. `fallback()` віддавав systemd власний текст помилки як аргумент: `"$@"`
   усередині функції — це аргументи **функції**, а не скрипта. systemd на
   зайвий аргумент виходить, а вихід PID 1 — паніка ядра. Тобто гілка «щось
   пішло не так, вантажимось без overlay» вбивала точку замість рятувати.
2. `mkdir /mnt/overlay` стояв **після** `remount,ro`. Ядро монтує корінь
   read-only з самого початку (`root_mountflags = MS_RDONLY`; rw робить уже
   systemd за fstab) — тобто mkdir падав **завжди**, а разом із (1) це давало
   картку, яка не завантажується взагалі. Каталог тепер лежить на картці
   готовим, а `mkdir -p` на наявному каталозі не пише нічого й не падає.
3. `chroot` у Raspbian лежить у `/usr/sbin`, а ядро передає init лише `HOME`
   і `TERM`, без `PATH`. Тепер PATH задається явно, а наявність
   `pivot_root`/`chroot`/`sed` перевіряється **до** першої незворотної дії.

Мораль не про конкретні рядки: скрипт, який виконується один раз на
завантаженні і якому нема куди впасти, читається проти **реального образу** —
чи є цей бінарник, чи є цей каталог, чи є модуль ядра, — а не проти
уявлення про те, як влаштований Debian.

### Побічні ефекти, з якими живемо

Системні журнали (`rsyslog`, journald) тепер пишуться в tmpfs і зникають
при ребуті. Наших логів це не стосується — вони на `data`; системний syslog
на кіоску ніхто не читав, а обсяг обмежений logrotate.

`/etc/fake-hwclock.data` замерз на даті образу, бо лежить на read-only
корені. Тобто механізм, яким Pi без RTC тримався за час, з переходом на
overlay помер: точка щоразу піднімається з тим самим моментом із минулого,
і розрив росте місяцями. Кривими графіками це не обмежується — з годинником
у минулому відмовляє HTTPS («сертифікат ще не дійсний»), а отже не їдуть ні
меню, ні оновлення (`../raspberry/pi/fix-clock.sh` описує це з натури).

**Полагоджено того ж дня** власною міткою на розділі `data` (`state/clock`,
код — `stack/common.sh`, розділ «Годинник»). Супервізор раз на хвилину
записує час, але **лише підтверджений NTP** — інакше один хибний час
запинився б назавжди; на старті, ще до запуску компонентів, піднімає
годинник до мітки, якщо той позаду. Це не заміна NTP, а закриття вікна до
його першої синхронізації: саме в цьому вікні проби лягали в чергу з
брехливим `measured_at`.

Чому не полагодили «як належить» — симлінком `/etc/fake-hwclock.data` на
`data`: `fake-hwclock.service` має `DefaultDependencies=no` і працює
`Before=sysinit.target`, тобто **до** монтування розділів. Симлінк у нікуди
дав би не старий час, а 1970 рік — гірше, ніж було. Переставляти порядок у
ранньому завантаженні заради цього не варте ризику.

Мітки мало, якщо точка піднялась **без інтернету**: годинник тоді йде від
моменту зникнення живлення, а проби лягають у чергу зі зсувом рівно на
довжину простою. NTP згодом пересуне час — і черга поїде на сервер із
брехливим `measured_at`, тобто на графіках виникне купа точок у минулому.
Тому `telemetry.sh` перештамповує чергу: кожна проба несе `uptime_s`, який
від годинника не залежить, і справжній час рахується як
`зараз − (аптайм зараз − аптайм проби)`. Коли годинник увесь час був
правильний, формула дає те саме значення, що вже стоїть у рядку, — тобто
перерахунок безпечний за побудовою. Чіпаються лише проби поточного
завантаження: `uptime` чужого ні про що не говорить.
...
**Ще не зроблено:** пʼять разів висмикнути живлення посеред роботи — і один
раз **посеред оновлення**. Щоразу точка має піднятись у меню сама. Цю
перевірку винесли на після переходу на overlay навмисно: тепер вона міряє
те, заради чого overlay і робили, а не витривалість записуваного кореня.
...
на точці), overseer обходить перевірки теж раз на хвилину
(`OVERSEER_INTERVAL_MS=60000`), а поріг мовчання сервер бере з РЕАЛЬНОГО
ритму точки — **два заміряні періоди плюс хвилина** (`checks.js`,
`silenceLimit`). Фіксоване число на сервері вже підвело: три хвилини проти
пʼятихвилинних проб дали маятник «мовчить — знову на звʼязку» кожні пʼять
хвилин. Типова затримка від поломки до повідомлення в Telegram —
**1–3 хвилини**.

Чому саме два періоди, а не три (виправлено 25.09.2026): телеметрія не
втрачає проб — не пішло, чекає в черзі й доїде наступним колом. Тому одна
невдала відправка не поломка, і поріг мусить бути більший за два періоди;
а дві поспіль — це вже справжній обрив. Заразом там знайшлась тихіша вада:
`PERIOD` спав ЗГОРИ на час самої проби, а вона коштує секунд чотирнадцять
(CEC до 12 с, `ping -c 5 -w 10`, секунда на /proc/stat) — тож «раз на 60 с»
насправді давало 74 с, поріг виходив 4,7 хв, і обрив на 4,5 хвилини
пролітав повз алерт. Тепер спимо стільки, скільки лишилось до кінця
періоду.

**Реліз доїжджає push-ом, опитування лишається бекапом** (25.09.2026).
Достукатись до точки ззовні не можна: вона за роутером у локальній мережі,
і маршруту до неї з GitHub Actions немає. Зате точка сама тримає ws до
прода — тим самим каналом, яким приїжджає `bonus_ready`. Ним і користуємось:

```
CI залив маніфест у бакет
   └─ той самий крок іде по ssh на дроплет (ключ для деплою там уже є)
        redis-cli publish points {"event":"pi_release"}
        └─ ws роздає це ВСІМ підключеним точкам (канал points)
             └─ кіоск (ws.c) торкається state/check-now
                  └─ updater.sh виходить з очікування за ≤5 с
```

Від публікації до початку оновлення — секунди. Кіоск тут лише листоноша:
оновленнями завідує апдейтер, просто ws тримає саме кіоск.

Чому окремий канал `points`, а не `point:<id>` кожній: інакше той, хто
шле, мусив би спершу спитати базу про живі точки — зайвий крок у CI або
список id, вписаний у воркфлоу руками. Канал один, ws роздає його всім
підключеним точкам.

Крок у CI навмисно **не валить збірку**: реліз уже в бакеті, і якщо
оповістити не вийшло (дроплет недоступний, redis перезапускається), точки
підхоплять його опитуванням. Це сповіщення, а не доставка.

Опитування лишається — це бекап на випадок, коли ws лежить, а HTTP працює.
Період — **2 хвилини** (`UPDATE_PERIOD_S`), а не пʼятнадцять: перевірка йде
з `If-None-Match` і без змін коштує 304 на пару сотень байтів, тож тримати
чверть години сліпої зони не було за що.

**Кіоск без мережі малює з кешу** (25.09.2026). Меню їде з бакета, і до
цього дня кіоск брав його ТІЛЬКИ звідти: після холодного старту без
інтернету йому не було чого малювати, шар лишався прозорим
(`gl_clear(!have_menu)`), і на екрані висіла запасна картинка `fbi` —
власник це й побачив, увімкнувши точку без кабелю. Тепер кожне вдале
`menu.json` лягає в `state/menu-cache.json` (через `.tmp` і `rename`), а на
старті без мережі читається звідти. Акція їде в тому самому файлі, тож
кешується разом із цінами.

**Як кіоск узагалі дізнається, що мережі немає.** Висмикнутий кабель сокет
не закриває — він просто замовкає. Тому ws шле пінг раз на 10 с і вважає
зʼєднання мертвим після 25 с тиші (`WS_PING_PERIOD_S`, `WS_IDLE_LIMIT_S`).
Саме ця пауза, а не витримка плашки, визначає момент її появи: було 25/60,
і плашка вилазила через 45 с замість обіцяних 15 (заміряв власник
25.09.2026). Разом тепер 25–40 с.

Бонуси, які вже висять на панелі, обрив не чіпає: зворотний відлік у
кожного QR свій і місцевий (`BONUS_TTL_S` мінус вік рядка), тож вони
спокійно дограють свої дві хвилини. Втрачаються лише НОВІ події — і рівно
про це плашка й попереджає. Єдина дрібниця: «телефон забрав бонус» теж
приходить по ws, тож без мережі забраний QR довисить до кінця таймера.
Шкоди немає — токен уже використаний, — а після перепідключення панель
вирівнюється по знімку стану точки.

Ціни з кешу такі самі вчорашні, як на запасній картинці, — різниця в тому,
що кіоск живий: малює свій кадр, тримає сокет телеметрії й покаже QR,
щойно підніметься ws. Але разом із меню малюється й **порожня панель
бонусів**, яка мовчки обіцяє те, чого без ws не буде. Тому після
**пʼятнадцяти секунд** без каналу на місці плашки «оновлення» зʼявляється
«Бонуси тимчасово недоступні — немає звʼязку» (`WS_OFFLINE_GRACE_S`,
`WS_OFFLINE_LABEL`). Спершу там стояло дві хвилини — власник резонно
зауважив, що ціна помилки несиметрична: плашка в порожньому кутку біля лого
не заважає нікому, а людина з купленою кавою не має дві хвилини чекати QR,
якого не буде. Менше пʼятнадцяти не варто: ws перепідключається за секунди,
і блимати на кожен розрив — це вже шум.
...
**«Мовчала» — це ще не «лежала», і ми вміємо їх розрізнити** (24.09.2026).
Малина не викидає проб при обриві: усе, що не пішло, лягає в
`state/telemetry.queue` і доїжджає потім зі **своїм** `measured_at`. Тому
після повернення в базі лишається два різні сліди:

| Що було | `measured_at` | `received_at` | Аптайм |
|---|---|---|---|
| Лежала мережа | суцільний ряд, дірки немає | пізніший на весь час обриву | росте далі |
| Лежала малина | **дірка**: міряти не було кому | звичайний | почався з нуля |
| Зависла телеметрія | **дірка** | звичайний | великий, без ребуту |

Тому повідомлення про повернення тепер не просто «знову на звʼязку», а
«не було інтернету 20 хв, малина працювала» / «малина не працювала 21 хв» /
«малина працювала, але не слала проб — щось зависало» (`checks.js`,
`outageKind`). Третій рядок найгірший із трьох: система жива, а частина її
тихо стала — це той випадок, який інакше проходить непоміченим.

Межі методу: черга тримає 500 проб (`QUEUE_MAX`), тобто близько восьми
годин при хвилинному ритмі — довший обрив мережі доїде обрізаним, і в
`measured_at` таки буде дірка на найстарішому. Класифікуємо по вікну в 12
годин; якщо точка мовчала довше, повідомлення лишається без пояснення, а
не з вигаданим.
...
| Знеструмлення точки | вимкнути живлення | телеметрія мовчить; у measured_at лишається дірка | 🔌 точка мовчить, потім ✅ «малина не працювала N хв» |
| Немає інтернету | вимкнути роутер | те саме мовчання, але проби доїжджають із черги й дірки в measured_at не лишається | 🔌 точка мовчить, потім ✅ «не було інтернету N хв, малина працювала» |
...
| Холодний старт без мережі | вимкнути живлення, вийняти кабель, увімкнути | меню береться з кешу на диску, ws не піднімається | на екрані меню (ціни з кешу) і плашка «Бонуси тимчасово недоступні» |
...
### Скільки насправді пишемо — заміряно 25.09.2026

Питання власника: «наскільки вистачить картки і чи можна оцінити її стан
зараз». Стан — не можна: SD-картка не віддає ні лічильника циклів, ні
залишку ресурсу (це вміють лише eMMC через `pre_eol_info`/`life_time`). У
`/sys/block/mmcblk0/device/` є тільки паспорт. Наша картка:

| | Картка до 25.09.2026 (тепер запасна) | Картка з 25.09.2026 |
|---|---|---|
| Модель | Toshiba `SA16G`, 16 ГБ (14,48 ГіБ) | Wibrand, 16 ГБ (14,56 ГіБ) |
| Дата випуску | **12/2017** — девʼятий рік | **01/2026** |
| Блок стирання | 4 МБ (`preferred_erase_size`) | 4 МБ |
| Розмітка | NOOBS: p1 1,8 ГБ · p5 32 МБ · p6 `/boot` 69 МБ · p7 `/` 12,7 ГБ | те саме + p7 ужато до 6 ГіБ і p8 `data` 6,6 ГБ (§5) |
| Запис іде | на корінь і в `/home/pi/extrovert` разом | **лише на `data`** |

Заміри нижче зроблені на старій картці — на новій вони не міняються: обсяг
запису задає софт, а не картка.

Зате **швидкість запису вимірюється точно** — сьомим полем
`/sys/block/mmcblk0/stat` (сектори, записані з моменту завантаження). Три
заміри по 5 хвилин у спокої дали таке:

| Звідки дивимось | Скільки |
|---|---|
| сума `write_bytes` усіх наших процесів | **17 МБ/добу** |
| що насправді пішло на картку | **292 МБ/добу** |
| те саме після `commit=300` | **54 МБ/добу** |

Різниця між 17 і 292 — це журнал ext4, який за замовчуванням скидається
**кожні пʼять секунд**. Наші записи дрібні (рядок у лог, рядок у чергу
телеметрії, файл стану), і на кожен такий байт журнал кладе свої блоки:
17280 комітів на добу. Один рядок у `/etc/fstab` — `commit=300` — прибирає
вісімнадцять із кожних двадцяти записаних мегабайтів. Платимо за це вікном
втрати: при знеструмленні зникне до пʼяти хвилин свіжих даних (файлова
система лишається цілою — журнал саме це й гарантує). Для нас це щонайбільше
пʼять проб телеметрії, які й так відновлюються.

Окремо: кожен реліз кіоска — це ~40 МБ (архів 10,8 МБ плюс розпаковані
ассети). У звичайний день релізів нуль, у день як 25.09.2026 — шість, і
тоді добова цифра стрибає до ~900 МБ. Це не привід не релізити, це привід
не тримати на картці десять старих релізів.

### Скільки вистачить: модель, а не вимір

Чесно: точної відповіді не існує, бо картка не каже про себе нічого. Але
порядок оцінити можна, і робимо це **песимістично**:

- запис на картку — 54 МБ/добу (після правки);
- множник контролера SD — **20×**: наші записи по 4 КБ лягають у блоки
  стирання по 4 МБ, і дешевий контролер у найгіршому разі стирає блок
  цілком. Реалістично 5–10, беремо 20;
- ресурс комірки — **300 циклів** (стара TLC; нова буває 500–1000);
- і головне: зношується не вся картка, а лише та частина, у яку пишуть.
  Дешеві контролери роблять *динамічне* вирівнювання — переставляють дані
  між ВІЛЬНИМИ блоками. Блоки, зайняті тим, що ніхто не переписує (система,
  ассети), у цій каруселі не беруть участі.

Звідси й головний висновок для overlayfs:

| Розмір частини, у яку пишемо | Ресурс | Протримається |
|---|---|---|
| 8,2 ГБ (як зараз) | 2,5 ТБ | **~6 років** |
| 4 ГБ | 1,2 ТБ | ~3 роки |
| 1 ГБ | 300 ГБ | **~9 місяців** |

Для порівняння, до правки журналу (292 МБ/добу) при теперішніх 8,2 ГБ
виходило б **~1,2 роки**.

**Тому розділ під змінні дані робимо максимально великим.** Коренева
система після переходу на overlayfs стає read-only й більше не росте: їй
вистачить 4–5 ГБ (зараз зайнято 3,5). Решта 12,7-гігабайтного p7 — під
змінні дані, тобто ~7,5 ГБ. Розділ на 1–2 ГБ «бо там лише логи» звів би
нанівець і overlayfs, і economію на журналі.

Чого НЕ чіпаємо: p1 RECOVERY на 1,8 ГБ. Спокуса забрати їх велика, але
саме звідти вантажиться прошивка (§1), і там лежить наш `disable_splash=1`.

### Що з цього випливає на практиці

1. `commit=300` уже в `/etc/fstab` (бекап — `/etc/fstab.bak-*`). Перевірено
   не перезавантаженням, а `mount -o remount /`, який читає опції саме з
   fstab: якщо застосувалось так, застосується й на старті.
2. Swap на старій картці був (99 МБ, `dphys-swapfile`), але за весь аптайм
   **не свопнуло жодної сторінки** (`vmstat -s`). На новій картці його
   немає взагалі: `swapon -s` порожній, `dphys-swapfile` — `inactive`,
   `/var/swap` відсутній. Так і треба. Під overlay swap-файл на корені
   опинився б у tmpfs, тобто «своп у памʼять» — витрата тієї самої RAM,
   заради якої свопають.
3. Найчесніша страховка — не арифметика, а **запасна картка з готовим
   образом**. Дев̓ятирічна картка з невідомою історією може померти завтра,
   і жодна оцінка цього не змінить.

### Overlay: увімкнено 25.09.2026

Головний важіль із цього списку — overlay на корінь — **зроблено**: як саме
й що при цьому вилізло, описано в §5. Для арифметики зносу важливий один
наслідок: після переходу на картку пише **лише розділ `data`**, і саме тому
його зробили 6,6 ГБ замість мінімально потрібних чотирьох.

Решта захисту від знеструмлення працювала й до того: ext4 із журналом,
симлінк релізу підміняється атомарно, запасна картинка пишеться через
`.tmp` + `rename`.
...
5. ~~**Реальний `curl` 7.52 на Stretch** проти публічного бакета~~ —
   закрито практикою 24–25.09.2026: точка за дві доби забрала з бакета
   пʼять релізів поспіль, з `If-None-Match`, `304` між ними й перевіркою
   sha. Заглушки більше ніде немає.
...
- ~~**Кадр стоїть, поки рендериться меню**~~ — зроблено 24.09.2026: рендер
  переїхав у фоновий потік. Далі 25.09.2026 розібрано й самі 32,8 с, які
  той потік займав. Заміряно на точці по частинах:

  | Що | Скільки |
  |---|---|
  | `render_menu` (rsvg, 13 карток) | 5,5 с |
  | запис запасної картинки (1920×1080 ARGB → PNG) | 7,1 с |
  | **разом чистої роботи** | **~13 с** |
  | а виходило у фоні | 32,8 с |

  Різниця — чиста конкуренція за єдине ядро з кадровим циклом. Тому лікували
  обидві половини: картинку більше не переписуємо, якщо поруч лежить хеш
  того самого меню (а платили ми ті 7 секунд на КОЖНОМУ старті кіоска,
  тобто на кожному релізі), а сам потік тепер `nice 10` — меню, намальоване
  на пів хвилини пізніше, не помітить ніхто, а просілі кадри помітно одразу.
- ~~**~43 fps замість 60, коли в панелі три бонуси**~~ — зроблено
  25.09.2026, і дешевше, ніж планувалось. Квад з обрізаними UV не
  знадобився: смуга 398 px вигоряє за дві хвилини, тобто повзе на ПІКСЕЛЬ
  раз на третину секунди, а перемальовувалась 60 разів на секунду. Тепер
  малюємо лише коли змінилась ціла ширина в пікселях — картинка та сама до
  пікселя, роботи у двадцять разів менше.

...
graph TB
    ...
    subgraph pub["Дроплет public (2 vCPU / 4 ГБ)"]
        ...
        SBOT["support-bot<br/>bostrot/telegram-support-bot"]
        MONGO[("MongoDB<br/>лише для бота")]
        PG[("Postgres")]
        RD[("Redis")]
        GT["GlitchTip"]
    end
    ...
    OVS --> PG
    OVS -->|"лише вихідні"| TG
    SBOT -->|"long polling, staff-група"| TG
    SBOT -->|"вебхуки тікетів, HMAC"| API
    SBOT --> MONGO
...
| `support-bot` + MongoDB | дроплет `public`, **без вхідних портів** | готовий бот підтримки забирає повідомлення long polling, а в `api` стукає внутрішньою мережею docker (§4) |
...
Вебхуки `support-bot` підписані: `X-TSB-Signature` — HMAC-SHA256 у hex
від тіла запиту з секретом із його `config.yaml`. Ендпоїнт у `api` слухає
лише внутрішню мережу docker, назовні його не видно.
...
### Підтримка через Telegram

Готове рішення, яке нічого не вимагає писати, крім прийому вебхуків, —
[`bostrot/telegram-support-bot`](https://github.com/bostrot/telegram-support-bot)
(v5, GPL-3.0, активно підтримується). Розгортається в нашому ж docker compose
на `public`: контейнер бота й MongoDB 8, яка потрібна йому для тікетів.
Обидва зібрано з тегу релізу, щоб оновлення бота були свідомими.

```
гравець → кнопка «Підтримка» → t.me/<бот>
  → support-bot (long polling) → тікет у Mongo → повідомлення в staff-групу Telegram
      → вебхук ticket.created → api → support_tickets → «Підтримка» в адмінці, лічильник +1
  → адмін відповідає реплаєм у staff-групі → бот пересилає гравцю
      → вебхук ticket.replied → лічильник −1
  → /close у staff-групі → ticket.closed
```

**Чому це drop-in.** Бот отримує оновлення Telegram через long polling
(у конфігу немає режиму вебхука, назовні публікується лише необовʼязковий
веб-порт) — вхідний порт на дроплеті йому не потрібен. Про тікети він сам
шле вебхуки (`ticket.created`, `ticket.replied`, `ticket.closed`,
`ticket.banned`, `csat.rated`, `ticket.escalated`) з payload
`{event, ticket_id, user_id, timestamp, message_preview, metadata}`, де
прев'ю — до 200 символів. Нашого коду — один ендпоїнт і таблиця
`support_tickets` (`db-schema.md` §4).

**Що варто знати наперед:**
- **Відповідають у staff-групі Telegram, а не в адмінці.** Розділ
  «Підтримка» — черга й лічильник з кнопкою «відкрити в Telegram».
  Відповідь з адмінки потребувала б API бота на відповідь, а його в коді
  бота не видно.
- **Ретраїв вебхуків у бота немає** (таймаут 5 с, помилка лише в лог).
  Ліг `api` — тікет в адмінку не потрапить, хоча в staff-групі він є.
  Staff-група лишається джерелом істини; якщо дірки стануть помітними —
  звірка з його MongoDB лише на читання.
- **Окремої події на повторне повідомлення гравця у відкритому тікеті в
  переліку немає**, тож лічильник рахує тікети без відповіді, а не
  повідомлення.
- MongoDB — ще одна база на дроплеті. Обмежити її кеш
  (`--wiredTigerCacheSizeGB 0.25`) і памʼять контейнера; бекапи не потрібні,
  поки копія тікетів живе в Postgres, а переписка — у staff-групі.

**Чому не інше:**
- **Chatwoot** дає повноцінну скриньку з відповіддю з вебу й нативним
  Telegram, але self-hosted — це Rails, Sidekiq, Postgres і Redis від 4 ГБ
  RAM, тобто окремий дроплет. У хмарному безкоштовному плані Telegram-каналу
  немає, він починається з платного тарифу за агента.
- **Livegram** і схожі хмарні конструктори безкоштовні, але не мають ні
  вебхуків, ні API — у адмінку звідти нічого не потрапить.
- **Zammad** важчий за Chatwoot (Elasticsearch), для однієї точки це
  непропорційно.

Якщо колись знадобиться відповідати саме з адмінки, наступний крок —
Chatwoot на окремому дроплеті, а розділ «Підтримка» стане його клієнтом
через API.
...
| `admin` | не існує |
| `support-bot` | не розгорнуто; готовий образ, нашого коду — ендпоїнт вебхуків і `support_tickets` |

...
Чотири бакети, поділені **за призначенням**, бо в них різні строки
зберігання, різні права й різні власники даних:

| Бакет | Що там | Хто пише | Хто читає | Строк |
|---|---|---|---|---|
| `extrovert-pos` | меню точок і релізи кіоска | `pos`-скрипти з машини розробника | будь-хто: публічний, `pos.extrovert.cafe` | доки не замінять |
...
**Оточення розділені іменем бакета.** Суфікс додає `lib/src/r2.js` за
змінною `APP_ENV`:
...
> ⚠️ Звідки взялося це правило. 20.09.2026 команда `prices:push`, запущена
...
провайдерами; тому `user_identities` окремою таблицею. **Пароля у гравців
немає взагалі:** це свідоме зняття з себе зберігання секретів.
...
- далі все як вище: той самий JWT на 15 хвилин і refresh-сесія на 12 годин;
...
**Вебсокет у C на Stretch — зроблено** (`pos-native/src/ws.c`, 21.09.2026).
...
`market_offer_bias` з `api/data/economy.json`, стартово 1: на кожне місце у
...
| `checkbox` | **вебхук із перевіркою підпису + опитування раз на хвилину**; чек → позиції → бонус → подія для кіоска в одній транзакції. Реєстрація самого вебхука в Checkbox ще попереду (потрібен `X-License-Key`) |
| `scheduler` | **написаний**: публікатор outbox, перенесення показів маркету, довідник і трекінг НП (обидві роботи НП чекають на `NP_API_KEY`) |
| `overseer` | **написаний**: мовчання точок, помилки вебхука Checkbox, щоденний звіт о 9:00; алерт лише на зміну стану |
| `lib` | спільна для сервісів: пул Postgres, Redis, outbox, блокування робіт, логер |
| `admin` | **заглушка-фронтенд** на Workers (`admin/`), дані бере роутом `/admin/overview`. Окремого сервіса в бекенді не має й не потребує: адмінка — це екрани, а не демон |
...
bun --watch api/src/index.js    # :3001
bun ws/src/index.js             # :3002
bun scheduler/src/index.js      # без порту
bun --cwd client run dev        # :5173
```
...
Кожен сервіс обробляє SIGTERM через `lib/src/shutdown.js`: спершу перестає
...
роботи зупиняються так само чесно: `every()` з `lib/src/jobs.js` повертає

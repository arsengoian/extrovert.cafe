...
курсори — `np_sync_cursors` у Postgres, тож дві копії сервісу не роблять те саме.
...
100 на сторінку. `from_date` — курсор із `checkbox_sync_cursors` мінус 10 хвилин:
...
`market_offer_bias` в `economy_params`, стартово 1: на кожне місце у

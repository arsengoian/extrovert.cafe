...
- **Refresh-сесія** — httpOnly-кука на `api.extrovert.cafe` і рядок
  ...
  грала). Вилогінює лише пів року тиші. Адміну — 12 годин без продовження.
...
| `qr.extrovert.cafe` | **наклейка надрукована** (`?p=1`), але хоста немає навіть у DNS (NXDOMAIN): скан зараз закінчується помилкою. Воркер `frontend/qr` написаний (22.09.2026) — DNS-запис і сертифікат зʼявляться з першим `make deploy-qr` (`urls.md`) |

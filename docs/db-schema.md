...
перемальовувати, свідомо немає.

---

## 0. Наскрізні рішення

**Гроші — `numeric(12,2)`, ігрові валюти — `integer`.** Гривня ніколи не
`float`; монети й зерна цілі за визначенням (економіка оперує `round(маржа)`,
`gamification_economy.md` §7).

**Ігрова валюта живе у двох місцях одночасно.** `wallets` — швидкий баланс
для UI, `ledger_entries` — незмінний журнал усіх рухів. Баланс завжди
похідний і звіряється з журналом; розбіжність — це баг, який видно, а не
тихо зіпсовані дані. Адмінка вимагає «історію транзакцій в справжній та
ігровій валюті» (`admin_panel.md`) — саме журнал це й дає.

**Косметика куща — `jsonb`, економіка — колонки.** У
`bush_graphics_customization.md` §9 стан кавенятка — великий документ
...
**Ідемпотентність скрізь, де ззовні.** Вебхуки ПРРО, телеметрія з малини,
заливка відеосегментів — усе має унікальний ключ від джерела: мережа на
точці рветься, і повтор запиту не має подвоювати ні чек, ні бонус.

**Точка (`point_id`) — текстовий ключ із першого дня** (`urls.md`),
`^[a-z0-9][a-z0-9-]{1,30}$`. Не uuid: він їде в URL кіоска й у ключі R2.
...
**Pub/sub канали** (доставка «в кращому разі», без збереження):
...
**Чому pub/sub, а не Streams.** Втрата повідомлення тут не втрачає дані:
...
Один статичний Go-бінарник (~10 МБ), чистий SQL, нічого не знає про
Node. Запускається окремим одноразовим контейнером, виходить — і все.

| | dbmate | node-pg-migrate | Atlas | Flyway/Liquibase | Sqitch |
|---|---|---|---|---|---|
| Окремо від сервісів | ✅ бінарник | ⚠️ тягне Node-залежності | ✅ | ✅ | ✅ |
| Формат | чистий SQL | JS або SQL | HCL/SQL, декларативно | SQL/XML | SQL + граф залежностей |
| Вага в CI | ~10 МБ | образ Node | ~30 МБ | JVM ~200 МБ | Perl |
| Down-міграції | ✅ | ✅ | ⚠️ інша модель | платно (Teams) | ✅ |
| Дамп схеми в git | ✅ вбудовано | ні | ✅ | ні | ні |
| Навчання | година | година | день | день | день |

`node-pg-migrate` — очевидний «залишитись у Node», але тоді міграції знову
поруч із сервісним `package.json`, і спокуса імпортнути з коду сервісу
зʼявиться сама. `Atlas` кращий, коли схема декларативна й потрібен
drift-detection — це наступний крок, не перший. JVM-інструменти на
дроплеті за $24 — невиправдана вага.
...
3. **`schema.sql` комітиться.** Рев'ю читає диф схеми, а не перебирає
   міграції очима.
4. **CI: накотити на чисту базу, потім накотити ще раз.** Друге накочення
   має бути no-op — це ловить міграції, які не ідемпотентні.
5. **Перший тестовий відкат — одразу**, як і з бекапами (`video.md`):
   `dbmate down` на стейджі до того, як він знадобиться на проді.

...
Каталог напоїв сьогодні живе в `pos/data/prices.json`, звідки меню йде в
R2. Коли зʼявиться база, його місце займе `db/seeds/drinks.json`, а меню для
кіоска збиратиме `scheduler` з таблиці (`services.md` §4).

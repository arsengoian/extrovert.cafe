...
- `scheduler`, `overseer`, `glitchtip` просто перезапускає: зовнішнього
  трафіку в них немає, і півхвилини без них не бачить ніхто;
- `caddy` не чіпає, поки не змінився його **образ за digest** — і це не
  дрібниця, а окремий крок (`caddy_step`).

> **Чому caddy окремо.** Він єдиний тримає 80/443: поки він перестворюється,
> назовні мовчать усі сервіси одразу. Навантажувальний тест 23.09.2026 зловив
> рівно це — 8,4 с без відповіді й три розриви ws, хоча api/ws/checkbox
> перемкнулись без жодної втрати. Причина була не в новому caddy: в імені
> образу стоїть `${TAG}`, тег міняється щодеплою, compose бачить інший рядок
> і перестворює контейнер — навіть коли за тим тегом лежить **той самий
> образ**, бо CI лише перевісив тег (`imagetools create`). Тепер порівнюється
> digest, і на звичайному деплої caddy не торкається ніхто.
>
> Якщо образ таки інший — це майже завжди правка `Caddyfile`. Скрипт дістає
> новий конфіг із нового образу, перевіряє його `caddy validate` і
> застосовує `caddy reload`: конфіг міняється на льоту, зʼєднання не рвуться
> (перевірено на проді 24.09.2026 — 537 запитів під час перезавантаження,
> жодної втрати, найгірша відповідь 272 мс). Контейнер при цьому лишається
> на старому образі — оновити сам caddy можна `RECREATE_CADDY=1`, і тоді
> кілька секунд простою це свідома плата, а не сюрприз.
...
Стан на 22.09.2026: заведені лише `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`,
`R2_SECRET_ACCESS_KEY` (реліз кіоска). Решти ще немає: деплой поки робиться
через act з машини розробника (нижче), а робота `deploy` у GitHub без них
упала б на ssh. Заводяться через API репозиторію (значення шифрується
публічним ключем репозиторію, sealed box libsodium) або в Settings → Secrets.

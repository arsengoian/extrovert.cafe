...
//
// Значення — середнє за добу, а не сума за місяць (у макеті «₴, середнє»):
// сума залежить від довжини проміжку, і два періоди поруч не порівняти.
const hours = (rows, days) => Array.from({ length: 24 }, (_, h) => ({
  label: String(h).padStart(2, "0"),
  value: Math.round((rows.find((r) => r.hour === h)?.sum_uah ?? 0) / Math.max(1, days)),
}));

// Для дня тижня дільник свій: за 30 діб понеділків чотири або п'ять, і
// ділити все на однакове число означало б занизити рідший день.

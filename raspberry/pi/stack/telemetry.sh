...
while [ "$STOPPING" -eq 0 ]; do
    ...
    # Спимо СКІЛЬКИ ЛИШИЛОСЬ до кінця періоду, а не цілий період згори.
    # Сама проба коштує секунд чотирнадцять: опит CEC до 12 с, ping -c 5 -w 10,
    # секунда на /proc/stat. Тому «раз на 60 с» насправді давало 74 с — і це
    # видно було не тут, а на сервері: поріг мовчання overseer рахується з
    # РЕАЛЬНОГО ритму точки, і замість очікуваних чотирьох хвилин виходило
    # майже пʼять. Обрив на 4,5 хвилини пролітав повз алерт (25.09.2026).
    _left=$(( PERIOD - ($(date +%s) - _started) ))
    # Проба довша за період — не крутимось без передиху: мінімум пʼять секунд
    # на те, щоб хоч щось інше на цій платі теж попрацювало.
    [ "$_left" -lt 5 ] && _left=5

...
# Прапорець `updating` знімає той, хто його підняв, — але не тоді, коли
# живлення зникає посеред підміни релізу. Тоді він лишається на картці, і
# точка піднімається з вічною плашкою «Оновлення» поверх меню, а супервізор
# іще й не перезапускає кіоск, поки прапорець висить (див. supervisor.sh).
# Тут він завідомо несвіжий: підняти його може лише апдейтер, а апдейтер
# щойно стартував. Самоперезапуск із нового релізу цього не зачіпає — там
# switch_to знімає прапорець ще до exec.
if [ -f "$UPDATING_FLAG" ]; then
    log "лишився прапорець «оновлення» від обірваного оновлення — знімаю"
    rm -f "$UPDATING_FLAG"
fi
